...
const Specimen = () => {
  ...
  async function getSpecimenData() {
    try {
      const respose = await fetch(`${REACT_APP_BASE_URL}specimen-radiology-master`)
      if (respose.ok) {
  ...
  const addSpecimen = (e) => {
    ...
    fetch(`${REACT_APP_BASE_URL}specimen-radiology-master`, {
      method: "POST",
      headers: {
  ...
  const deleteSpecimen = async (id) => {
    try {
      const response = await fetch(`${REACT_APP_BASE_URL}specimen-radiology-master/delete/${id}`, {
        method: 'PUT',
      });
      if (response.ok) {
        getSpecimenData()
