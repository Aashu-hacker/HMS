const { UnitMasterModel } = require('../../../models');
...
const createUnit = async (req, res) => {
    try {
        ...
        const { name } = req.body
        const unit = new UnitMasterModel({
            name: name,
        });
        await unit.save();
...
const getAllUnits = async (req, res) => {
    try {
        const units = await UnitMasterModel.find({ delete: false });
...
const getUnitById = async (req, res) => {
    try {
        const { id } = req.params
        const unit = await UnitMasterModel.findById({ _id: id });
...
const updateUnitById = async (req, res) => {
    try {
        ...
        const unit = await UnitMasterModel.findByIdAndUpdate(
            { _id: id },
            {
                name: req.body.name,
                updatedAt: Date.now(),
...
const deleteUnitById = async (req, res) => {
    try {
        const { id } = req.params
        const unit = await UnitMasterModel.findByIdAndUpdate({ _id: id }, { ...req.body, delete: true, deletedAt: Date.now(), new: true });
        if (!unit) {
            return res.status(httpStatus.NOT_FOUND).json({ error: 'Unit not found' });
        }
