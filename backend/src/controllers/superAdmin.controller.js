...
const { registerValidation, loginValidation } = require('../validations/superAdmin.validations');
...
const register = async (req, res) => {
    ...
    if (emailExists) {
        return res.status(httpStatus.BAD_REQUEST).json({ msg: 'Super admin already exists' });
    }
    ...
    try {
        ...
        res.status(httpStatus.CREATED).json({ msg: 'Super-Admin registered successfully!!!', superAdmin: savedAdmin });
    } catch (err) {
...
const login = async (req, res) => {
    ...
    if (!admin) {
        return res.status(httpStatus.BAD_REQUEST).json({ msg: 'Email or password is incorrect' });
    }
    ...
    res.status(httpStatus.OK).json({ msg: `Welcome ${admin.name}`, token: token, userId: admin._id, Email: admin.email, role: admin.role });
};
...
const getSuperAdmin = async (req, res) => {
    try {
        ...
        res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error retrieving subadmins' });
    }
}
...
const deleteSuperAdmin = async (req, res) => {
    try {
        ...
        res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Super Admin deleted successfully' });
    } catch (error) {
        res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error deleting super Admin' });
    }
}
